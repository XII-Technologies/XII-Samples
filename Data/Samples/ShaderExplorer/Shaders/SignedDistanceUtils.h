...
// A list of useful distance function to simple primitives. All these functions (except for ellipsoid) return an exact
// euclidean distance, meaning they produce a better SDF than what you'd get if you were constructing them from boolean
...
{
  float2 d = abs(float2(length(float2(p.x, p.z)), p.y)) - h;
  return min(max(d.x, d.y), 0.0) + length(max(d, 0.0));
}
...
{
  float2 p = float2(length(float2(pos.x, pos.z)), pos.y);
  float  l = length(p) - ra;
...
float sdRhombus(float3 p, float la, float lb, float h, float ra)
{
  ...
  float  f = clamp((ndot(b, b - 2.0 * float2(p.x, p.z))) / dot(b, b), -1.0, 1.0);
  float2 q = float2(length(float2(p.x, p.z) - 0.5 * b * float2(1.0 - f, 1.0 + f)) * sign(p.x * b.y + p.z * b.x - b.x * b.y) - ra, p.y - h);
